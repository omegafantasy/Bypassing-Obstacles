...
public class GlobalConfiguration : MonoBehaviour
{
    ...
    public enum PathSeedChoice { _90Turn, RandomTurn, StraightLine, Sawtooth, Circle, FigureEight, FilePath, RealUserPath, VEPath, ValidPath, Irregular };
    ...
    [Tooltip("The time step for short-term path prediction")]
    public float timeStep = 1f;
    [Tooltip("The number of direction samples")]
    ...
    public Vector2 currDir2D, currPos2D;
    ...
    [Tooltip("Total physical path length for each avatar")]
    [Range(50, 600)]
    ...
    [HideInInspector]
    public Transform virtualSpaceObject;
    ...
    // redirection decision phase for physical arc
    public void PhysicalARCRedirectionDecision()
    {
        List<float> scores = new List<float>();
        // List<float> safetyValues = new List<float>();
        float baseScore;
        bool someoneRotating = false;
        for (int i = 0; i < avatarNum; i++)
        {
            var rm = redirectedAvatars[i].GetComponent<RedirectionManager>();
            var rd = (PhysicalARC_Redirector)(rm.redirector);
            if (rm.isRotating || rm.touchWaypoint)
            {
                someoneRotating = true;
            }
            scores.Add(rd.currScore);
            // safetyValues.Add(rd.angle2Waypoint / rotationSpeed + rd.straightlineDis / translationSpeed);
        }
        scores.Sort();
        if (avatarNum % 2 == 0)
        {
            baseScore = (scores[avatarNum / 2] + scores[avatarNum / 2 - 1]) / 2;
        }
        else
        {
            baseScore = scores[avatarNum / 2];
        }
        for (int i = 0; i < avatarNum; i++)
        {
            var rm = redirectedAvatars[i].GetComponent<RedirectionManager>();
            ...
            float targetGc = 1;
            if (rm.isRotating || rm.touchWaypoint)
            { // the avatar is rotating
                if (rd.aheadStraightDis / rd.backStraightDis > 2)
                { // turn slower
                    targetGc = MAX_ROT_GAIN;
                }
                else if (rd.backStraightDis / rd.aheadStraightDis > 2)
                { // turn faster
                    targetGc = MIN_ROT_GAIN;
                }
                targetGt = MAX_TRANS_GAIN;
            }
            else if (someoneRotating)
            { // another avatar is rotating
                // Debug.Log("rotate");
                float maxValue = -1;
                foreach (var res in rd.pathPredictionResults)
                {
                    if (res.Item5 > maxValue)
                    {
                        maxValue = res.Item5;
                        targetGt = res.Item1;
                        targetCurvature = res.Item2;
                    }
                }
            }
            else
            { // everyone walk normally
                float minDiff = INF;
                foreach (var res in rd.pathPredictionResults)
                {
                    if (Mathf.Abs(res.Item5 - baseScore) < minDiff)
                    {
                        minDiff = Mathf.Abs(res.Item5 - baseScore);
                        targetGt = res.Item1;
                        targetCurvature = res.Item2;
                    }
                }
            ...
            rd.redirectionUnitARC = new Tuple<float, float, float>(targetGt, targetCurvature, targetGc);
            rd.useRedirectionUnit = true;
        }
    }

    // reset decision phase for physical arc
    public void PhysicalARCResetDecision()
    {
        List<List<float>> all_scores = new List<List<float>>();
        List<float> maxScores = new List<float>();
        float baseScore = INF;
        for (int i = 0; i < avatarNum; i++)
        {
            var rm = redirectedAvatars[i].GetComponent<RedirectionManager>();
            ...
            float maxScore = -1;
            all_scores.Add(new List<float>());
            for (int j = 0; j < directionSamples; j++)
            {
                float dirAngle = (2 * Mathf.PI * j) / directionSamples;
                Vector2 dir = new Vector2(Mathf.Cos(dirAngle), Mathf.Sin(dirAngle));
                if (rm.IsDirSafe(Utilities.FlattenedPos2D(rm.currPosReal), dir))
                {
                    var newpos = Utilities.FlattenedPos2D(rm.currPosReal) + dir * 0.25f;
                    var ahead = rm.GetPhysicalDistance(newpos, dir);
                    var left = rm.GetPhysicalDistance(newpos, Utilities.RotateVector(dir, -90));
                    var right = rm.GetPhysicalDistance(newpos, Utilities.RotateVector(dir, 90));
                    var nowScore = Mathf.Pow(ahead, 3f / 4) * Mathf.Pow(left * right, 1f / 8) / Mathf.Sqrt(rm.area);
                    // var nowScore = ahead;
                    if (nowScore > maxScore)
                    {
                        maxScore = nowScore;
                    }
                    all_scores[i].Add(nowScore);
                }
                else
                {
                    all_scores[i].Add(-1);
        ...
        for (int i = 0; i < avatarNum; i++)
        {
            baseScore = Mathf.Min(maxScores[i], baseScore);
        }
        for (int i = 0; i < avatarNum; i++)
        {
            float minDiff = INF;
            ...
            for (int j = 0; j < directionSamples; j++)
            {
                if (Mathf.Abs(all_scores[i][j] - baseScore) < minDiff)
                {
                    minDiff = Mathf.Abs(all_scores[i][j] - baseScore);
                    targetAngle = (2 * Mathf.PI * j) / directionSamples;
                }
            }
            // Debug.Log(i + ":" + targetAngle);
            var rs = (PhysicalARC_Resetter)(redirectedAvatars[i].GetComponent<RedirectionManager>().resetter);
            rs.resetDir = new Vector2(Mathf.Cos(targetAngle), Mathf.Sin(targetAngle));
            rs.useResetDir = true;
        }
    }
    ...
    // redirection & reset phase for bypassing redirector
    public void BypassingDecision(List<int> indexes)
    {
        List<RedirectionManager> managers = new List<RedirectionManager>();
        List<Bypassing_Redirector> redirectors = new List<Bypassing_Redirector>();
        List<int> resetAvatars = new List<int>();
        List<int> safeAvatars = new List<int>();
        List<int> rotatingAvatars = new List<int>();
        int n = avatarNum;
        bool triggerSyncReset = false;
        bool someoneRotating = false;
        for (int i = 0; i < n; i++)
        {
            managers.Add(redirectedAvatars[i].GetComponent<RedirectionManager>());
            if (managers[i].isRotating)
            {
                someoneRotating = true;
            }
            redirectors.Add((Bypassing_Redirector)(managers[i].redirector));
        }
        ...
        { // when sync reset
            ...
            {//need to reset
                ...
                foreach (var id in safeAvatars)
                {
                    if (timeToReset > redirectors[id].timeToWayPoint)
                    {
                        redirectors[id].ResetAndGuideToSafePos();
                    }
                    else
                    {
                        redirectors[id].ResetAndGuideToMaxTimePos(timeToReset);
        ...
        { // when someone hit a waypoint, or performing routine decision
            for (int i = 0; i < n; i++)
            {
                if (indexes.Contains(i) || managers[i].isRotating)
                {
                    rotatingAvatars.Add(i);
                }
                else if (redirectors[i].collisionTimeRange.Item2 <= redirectors[i].timeToWayPoint)
                {// max<=T
                    resetAvatars.Add(i);
                }
                else
            ...
            foreach (var id in rotatingAvatars)
            {

                if (managers[id].isRotating)
                {
                    // Debug.Log(id+"rotate");
                    redirectors[id].GuideToFurthest();
                }
                ...
                {
                    // Debug.Log(id+"touch");
                    redirectors[id].RotateToBestAngle();
                }
            }
            if (resetAvatars.Count == 0)
            {//no need to reset
            ...
            {//need to reset
                ...
                foreach (var id in safeAvatars)
                {
                    if (timeToCollide > redirectors[id].timeToWayPoint)
                    {
                        redirectors[id].GuideToSafePos();
                    }
                    else
        ...
        // Debug.Log((redirectors[0].targetPosition != null ? redirectors[0].targetPosition.center : new Vector2(INF, INF)) + " " + redirectors[0].redirectParams.Item1 + " " + redirectors[0].useTurnPoint + " " + redirectors[0].turnPoint);
    }
    ...
    void Update()
    {
        ...
        UpdateUI();
        currDir2D = Utilities.FlattenedDir2D(redirectedAvatars[0].GetComponent<RedirectionManager>().currDirReal);
        currPos2D = Utilities.FlattenedPos2D(redirectedAvatars[0].GetComponent<RedirectionManager>().currPosReal);
        var deltaDis = redirectedAvatars[0].GetComponent<RedirectionManager>().deltaPos.magnitude;
        if (deltaDis / Time.deltaTime > 0.3f && !redirectedAvatars[0].GetComponent<RedirectionManager>().inReset)
        {
            walkDist += deltaDis;
        }
    }
    ...
    public void MakeOneStepRedirection()
    {
        ...
        List<int> touchWaypointIndexes = new List<int>();
        for (int i = 0; i < avatarNum; i++)
        {
            ...
            else if (remanager.resetter != null && !remanager.inReset && remanager.resetter.IsResetRequired() && remanager.EndResetCountDown == 0)
            {
                ...
                if (redirectedAvatars[0].GetComponent<RedirectionManager>().redirectorChoice == RedirectionManager.RedirectorChoice.PhysicalARC)
                    PhysicalARCResetDecision();
                break;
            }
        }
        ...
        else if (redirectedAvatars[0].GetComponent<RedirectionManager>().redirectorChoice == RedirectionManager.RedirectorChoice.Bypassing)
        {
            timeStepCountDown--;
            if (touchWaypoint || triggerSyncReset || timeStepCountDown <= 0)
            {
                timeStepCountDown = timeStepBase;
                for (int i = 0; i < avatarNum; i++)
                {
                    RedirectionManager rm = redirectedAvatars[i].GetComponent<RedirectionManager>();
                    Bypassing_Redirector rd = (Bypassing_Redirector)(rm.redirector);
                    rd.collisionParams = rd.GetCollisionParams(new Vector2(rm.currPosReal.x, rm.currPosReal.z), Utilities.FlattenedPos2D(Utilities.GetRelativePosition(rm.targetWaypoint.position, rm.trackingSpace.transform) - rm.currPosReal));
                    if (triggerSyncReset)
                    {
                        rd.resetTimeRange = rd.GetResetTimeRange(new Vector2(rm.currPosReal.x, rm.currPosReal.z));
                ...
                BypassingDecision(touchWaypointIndexes);
            }
        }
        else if (redirectedAvatars[0].GetComponent<RedirectionManager>().redirectorChoice == RedirectionManager.RedirectorChoice.PhysicalARC)
        {
            if (triggerSyncReset)
            {
                timeStepCountDown = 1;
            ...
            {
                timeStepCountDown--;
                if (timeStepCountDown <= 0 || touchWaypoint)
                {
                    timeStepCountDown = timeStepBase;
                    for (int i = 0; i < avatarNum; i++)
                    {
                        var rd = (PhysicalARC_Redirector)(redirectedAvatars[i].GetComponent<RedirectionManager>().redirector);
                        rd.ShortTermPathPrediction();
                    }
                    PhysicalARCRedirectionDecision();
                }
            }
        }
        for (int i = 0; i < avatarIdSortedFromHighPriorityToLow.Count; i++)
    ...
    public static PathSeedChoice DecodePathSeedChoice(string s)
    {
        switch (s.ToLower())
        {
            ...
            case "irregular":
                return PathSeedChoice.Irregular;
            default:
                return PathSeedChoice.Sawtooth;
        }
    }
        ...
        else
        {
            ...
            switch (pathSeedChoice)
            {
                ...
                case PathSeedChoice.Sawtooth:
                    ...
                    break;
                case PathSeedChoice.Irregular:
                    waypoints = VirtualPathGenerator.GenerateInitialPathByPathSeed(randomSeed, PathSeed.GetPathSeedIrregular(), pathLength, out sumOfDistances, out sumOfRotations);
                    break;
