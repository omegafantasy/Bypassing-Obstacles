...
public static class Utilities
{
    ...
    public static List<Vector2> BufferPolygon(List<Vector2> polygon, float buffer, bool isSpace)
    {
        var res = new List<Vector2>();
        for (int i = 1; i <= polygon.Count; i++)
        {
            Vector2 p = polygon[i - 1], q = polygon[i % polygon.Count], r = polygon[(i + 1) % polygon.Count];
            bool inSign = isSpace;
            if (GetSignedAngle(q - p, r - q) > 0)
            {
                inSign = !inSign;
            }
            if (inSign)
            {
                res.Add(q - buffer * (q - p).normalized + buffer * (r - q).normalized);
            }
            else
            {
                res.Add(q + buffer * (q - p).normalized - buffer * (r - q).normalized);
    ...
    public static bool GetCommandFilePath(out string path)
    {
        ...
        ofn.initialDir = "./CommandFiles";//default path
    ...
    public static float CalculatePolygonArea(List<Vector2> points)
    {
        ...
        for (int i = 0; i < len; i++)
        {
            res += points[i].x * (points[(i + 1) % len].y - points[(i + len - 1) % len].y);
        }
        res /= 2;
        return res;
    }
