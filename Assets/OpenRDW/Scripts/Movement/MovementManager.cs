...
public class MovementManager : MonoBehaviour
{
    ...
    public void LoadData(int avatarId, AvatarInfo avatar)
    {
        ...
        rm.bufferedPolygons = new List<List<Vector2>>();
        float buffer = generalManager.RESET_TRIGGER_BUFFER + 0.05f;
        rm.polygons.Add(generalManager.physicalSpaces[physicalSpaceIndex].trackingSpace);
        rm.bufferedPolygons.Add(Utilities.BufferPolygon(generalManager.physicalSpaces[physicalSpaceIndex].trackingSpace, buffer, true));
        rm.area = Utilities.CalculatePolygonArea(generalManager.physicalSpaces[physicalSpaceIndex].trackingSpace);
        foreach (var obs in generalManager.physicalSpaces[physicalSpaceIndex].obstaclePolygons)
        {
            rm.polygons.Add(obs);
            rm.bufferedPolygons.Add(Utilities.BufferPolygon(obs, buffer, false));
            rm.area -= Utilities.CalculatePolygonArea(obs);
        }
        rm.polygonEdges = new List<Tuple<Vector2, Vector2>>();
        foreach (var polygon in rm.bufferedPolygons)
        {
            for (int i = 0; i < polygon.Count; i++)
            {
                rm.polygonEdges.Add(new Tuple<Vector2, Vector2>(polygon[i], polygon[(i + 1) % polygon.Count]));
            }
        }
        ...
        else if (rm.redirectorChoice == RedirectionManager.RedirectorChoice.Bypassing)
        {
            ((Bypassing_Redirector)rm.redirector).InitializePositionSquares();
        }
