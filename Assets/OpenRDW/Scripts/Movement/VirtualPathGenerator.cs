...
public class VirtualPathGenerator
{
    ...
    public struct PathSeed
    {
        ...
        public SamplingDistribution distanceDistributionShort; // for irregular sampler
        public SamplingDistribution angleDistributionShort; // for irregular sampler
        public int type; // 0 for regular samplers, 1 for a novel irregular sampler
        public PathSeed(SamplingDistribution distanceDistribution, SamplingDistribution angleDistribution, int waypointCount)
        {
            ...
            this.type = 0;
            this.distanceDistributionShort = distanceDistribution;
            this.angleDistributionShort = angleDistribution;
        }
        public PathSeed(SamplingDistribution distanceDistribution, SamplingDistribution angleDistribution, int waypointCount,
             SamplingDistribution distanceDistributionShort, SamplingDistribution angleDistributionShort)
        {
            this.distanceDistribution = distanceDistribution;
            this.angleDistribution = angleDistribution;
            this.waypointCount = waypointCount;
            this.type = 1;
            this.distanceDistributionShort = distanceDistributionShort;
            this.angleDistributionShort = angleDistributionShort;
        }
        public static PathSeed GetPathSeedIrregular()
        {
            ...
            SamplingDistribution distanceSamplingDistributionShort = new SamplingDistribution(DistributionType.Uniform, 0.5f, 1f);
            SamplingDistribution angleSamplingDistributionShort = new SamplingDistribution(DistributionType.Uniform, -135, 135);
            int waypointCount = 50;
            return new PathSeed(distanceSamplingDistribution, angleSamplingDistribution, waypointCount,
                distanceSamplingDistributionShort, angleSamplingDistributionShort);
        }
    ...
    static float SampleIrregularDistance(PathSeed seed, int status)
    {
        if (status > 0)
        {
            return SampleUniform(seed.distanceDistribution.min, seed.distanceDistribution.max);
        }
        else
        {
            return SampleUniform(seed.distanceDistributionShort.min, seed.distanceDistributionShort.max);
        }
    }
    static float SampleIrregularRotation(PathSeed seed, int status)
    {
        if (status > 0)
        {
            return SampleUniform(seed.angleDistribution.min, seed.angleDistribution.max);
        }
        else
        {
            return SampleUniform(seed.angleDistributionShort.min, seed.angleDistributionShort.max);
        }
    }
    ...
    public static List<Vector2> GenerateInitialPathByPathSeed(int randomSeed, PathSeed pathSeed, float targetDist, out float sumOfDistances, out float sumOfRotations)
    {
        ...
        for (; !finished;)
        {
            ...
            {
                sampledDistance = SampleIrregularDistance(pathSeed, irregularStatus);
            }
            ...
            {
                sampledRotation = SampleIrregularRotation(pathSeed, irregularStatus);
                if (irregularStatus > 0)
                {
                    irregularStatus--;
                    if (irregularStatus == 0)
                    {
                        irregularStatus = -Random.Range(4, 7);
                    }
                }
                else if (irregularStatus < 0)
                {
                    irregularStatus++;
                    if (irregularStatus == 0)
                    {
                        irregularStatus = Random.Range(3, 6);
                    }
                }
            }
