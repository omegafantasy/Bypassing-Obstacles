...
public class SimulatedWalker : MonoBehaviour
{
    ...
    {
        Random.InitState((int)System.DateTime.Now.Ticks + movementManager.avatarId);
    }
    ...
    {
        // if (speedRatio > 0.8f && Random.Range(0f, 1f) < globalConfiguration.GetDeltaTime())
        // {
        //     speedRatio = 0.5f;
        // }
        // else if (speedRatio < 0.8f && Random.Range(0f, 1f) < globalConfiguration.GetDeltaTime() * 2)
        // {
        //     speedRatio = 1f;
        // }
    ...
    public void WalkIfPossible(float rotationToTargetInDegrees, Vector3 userToTargetVectorFlat)
    {
        ...
        if (Mathf.Abs(rotationToTargetInDegrees) < ROTATIONAL_ERROR_ACCEPTED_IN_DEGRESS)
        {
            ...
            float distanceToTravel = redirectionManager.GetDeltaTime() * globalConfiguration.translationSpeed * speedRatio;
            ...
            else if (redirectionManager.redirectorChoice == RedirectionManager.RedirectorChoice.Bypassing)
            {
                var rd = (Bypassing_Redirector)redirectionManager.redirector;
                if (rd.useRedirectParams)
                {
                    distanceToTravel *= 1 / rd.redirectParams.Item3; // gt
                }
            }
            else if (redirectionManager.redirectorChoice == RedirectionManager.RedirectorChoice.PhysicalARC)
            {
                var rd = (PhysicalARC_Redirector)redirectionManager.redirector;
                if (rd.useRedirectionUnit)
                {
                    distanceToTravel *= 1 / rd.redirectionUnitARC.Item1; // gt
                }
            }
