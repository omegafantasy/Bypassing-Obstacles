...
public abstract class Resetter : MonoBehaviour
{
    ...
    public void UpdatePanel()
    {
        ...
        if (redirectionManager.inReset)
        {
            ...
            if (distanceVector.magnitude < 0.16f &&
                arrow.localRotation.eulerAngles.magnitude < 12.0f)
    ...
    public bool IfCollisionHappens()
    {
        ...
        var realDir = Utilities.FlattenedPos2D(redirectionManager.deltaPosReal);
        if (realDir.magnitude < 1e-3)
        {
            realDir = new Vector2(redirectionManager.currDirReal.x, redirectionManager.currDirReal.z);
        }
        else
        {
        ...
        //var polygons = new List<List<Vector2>>(); //jon: we no longer use this polygons, to save memory use
    ...
    public Vector2 DecideResetPosition(Vector2 currPosReal)
    {
        ...
        var dis_pos = Utilities.GetNearestDistAndPosToObstacleAndTrackingSpace(globalConfiguration.physicalSpaces, movementManager.physicalSpaceIndex, currPosReal);
        if (dis_pos.Item1 > globalConfiguration.RESET_TRIGGER_BUFFER + 0.15f)
        {
            return currPosReal;
        }
        else
        {
            return currPosReal + (currPosReal - dis_pos.Item2).normalized * (globalConfiguration.RESET_TRIGGER_BUFFER + 0.15f - dis_pos.Item1);
        }
    }
